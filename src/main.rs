use ::image as im;
use cgmath::{dot, Vector3, InnerSpace, ElementWise};
use rand::Rng;
...
const VFOV: f64 = 20.;
const LOOKFROM: Vector3<f64> = Vector3 {x: 13., y: 2., z: 3.};
const LOOKAT: Vector3<f64> = Vector3 {x: 0., y: 0., z: 0.};
const VUP: Vector3<f64> = Vector3 {x: 0., y: 1., z: 0.};
const DEFOCUS_ANGLE: f64 = 0.;
const FOCUS_DIST: f64 = 10.;

//const VFOV: f64 = 40.;
//const LOOKFROM: Vector3<f64> = Vector3 {x: 800., y: 0., z: 0.};
//const LOOKAT: Vector3<f64> = Vector3 {x: 0., y: 0., z: 0.};
//const VUP: Vector3<f64> = Vector3 {x: 0., y: 1., z: 0.};
//const DEFOCUS_ANGLE: f64 = 0.;
//const FOCUS_DIST: f64 = 10.;
...
fn random_vec_from_interval(i: Interval) -> Vector3<f64> {
    Vector3::from([random_in_interval(i), random_in_interval(i), random_in_interval(i)])
}

fn random_vec_in_unit_sphere() -> Vector3<f64> {
    loop {
        let p = random_vec_from_interval(Interval::from(-1., 1.));
        if p.magnitude2() < 1. {return p}
    }
}
...
fn random_in_unit_disk() -> Vector3<f64> {
    loop {
        let p = Vector3::from([random_in_interval(Interval::from(-1., 1.)),random_in_interval(Interval::from(-1., 1.)),0.]);
        if p.magnitude2() < 1. {return p}
    }
}
...
fn rgb_to_color(mut r: f64, mut g: f64, mut b: f64, samples: i32) -> [u8; 4] {
    ...
    [(255.99 * intensity.clamp(r)) as u8, (255.99 * intensity.clamp(g)) as u8, (255.99 * intensity.clamp(b)) as u8, 255]
}
...
impl Material {
    fn scatter(&self, ray: Ray, rec: HitRecord, attenuation: &mut Vector3<f64>, scattered: &mut Ray) -> bool{
        match self.material_type {
            ...
            MaterialType::Dielectric(ir) => {
                *attenuation = Vector3::from([1.,1.,1.]);
                ...
                {direction = reflect(unit_direction, rec.normal)} else 
...
impl HitRecord {
    fn new() -> HitRecord {
        HitRecord {position: Vector3::from([0.,0.,0.]), normal: Vector3::from([0.,0.,0.]), material: Material {material_type: MaterialType::None}, t: 0., front_face: true}
    }
...
impl World {
    ...
    fn hit_spheres(&self, r: Ray, ray_t: Interval, rec: &mut HitRecord) -> bool {
        ...
        for sphere in &self.spheres {
            if r.hit_sphere(sphere, Interval::from(ray_t.min, closest_so_far), &mut temp_rec) {
                hit_anything = true;
...
fn ray_color(r: Ray, depth: i32, world: World) -> Vector3<f64> {
    ...
    if depth <= 0 {return Vector3::from([0.,0.,0.])}
    ...
    if world.hit_spheres(r, Interval::from(0.001, f64::INFINITY) , &mut rec) {
        ...
        let color_from_scatter = attenuation.mul_element_wise(ray_color(scattered, depth-1, world));
...
fn render(canvas: &mut im::ImageBuffer<im::Rgba<u8>, Vec<u8>>, world: World, samples_per_pixel: i32, max_depth: i32) {
    ...
    for x in 0..WINDOW_WIDTH {
        for y in 0..WINDOW_HEIGHT {
            ...
            for _ in 0..samples_per_pixel {
                let r = get_ray(x, y, camera_position, pixel00_loc, pixel_delta_u, pixel_delta_v, defocus_disk_u, defocus_disk_v);
                
                //dbg!(pixel_color, sample_color);
                pixel_color += ray_color(r, max_depth, world.clone());
            }
            let color = rgb_to_color(pixel_color.x, pixel_color.y, pixel_color.z, samples_per_pixel);
            canvas.put_pixel(x, y, im::Rgba(color));
        }
    }          
}

fn get_ray(x: u32, y: u32, camera_position: Vector3<f64>, pixel00_loc : Vector3<f64>, pixel_delta_u: Vector3<f64>,
     pixel_delta_v: Vector3<f64>, defocus_disk_u: Vector3<f64>, defocus_disk_v: Vector3<f64>) -> Ray {
    ...
    let ray_origin = if DEFOCUS_ANGLE <= 0. {camera_position} else {defocus_disk_sample(camera_position, defocus_disk_u, defocus_disk_v)};
    let ray_direction = pixel_sample - ray_origin;

    Ray::new(camera_position, ray_direction)
}

fn defocus_disk_sample(position: Vector3<f64>, defocus_disk_u: Vector3<f64>, defocus_disk_v: Vector3<f64>) -> Vector3<f64> {
...
fn main() {
    ...
    world.spheres.push(Sphere { position: Vector3::from([0., -1000., 0.]), radius: 1000., material: Material {material_type: material_ground}});

    for a in -11..11 {
        for b in -11..11 {
            let choose_material = random_0_1();
            let center = Vector3::from([a as f64 + 0.9*random_0_1(), 0.2, b as f64 + 0.9 * random_0_1()]);

            if (center - Vector3::from([4.,0.2,0.])).magnitude() > 0.9 {
                if choose_material < 0.4 {
                    let light = MaterialType::DiffuseLight;
                    world.spheres.push(Sphere { position: center, radius: 0.2, material: Material {material_type: light}});
                }
                if choose_material < 0.8 {
                    let color = Vector3::from([random_in_interval(Interval::from(0., 1.)), 
                    random_in_interval(Interval::from(0., 1.)), random_in_interval(Interval::from(0., 1.))]);
                    let sphere_material = MaterialType::Lambertian(color);
                    world.spheres.push(Sphere { position: center, radius: 0.2, material: Material {material_type: sphere_material}});
                }
                else if choose_material < 0.95 {
                    let color = Vector3::from([random_in_interval(Interval::from(0.5, 1.)), 
                    random_in_interval(Interval::from(0., 1.)), random_in_interval(Interval::from(0., 1.))]);
    ...
    world.spheres.push(Sphere { position: Vector3::from([0.,1.,0.]), radius: 1., material: Material {material_type: material1}});

    let material2 = MaterialType::Lambertian(Vector3::from([0.4,0.2,0.1]));
    world.spheres.push(Sphere { position: Vector3::from([-4.,1.,0.]), radius: 1., material: Material {material_type: material2}});

    let material3 = MaterialType::Metal(Vector3::from([0.7,0.6,0.5]), 0.0);
    world.spheres.push(Sphere { position: Vector3::from([4.,1.,0.]), radius: 1., material: Material {material_type: material3}});

    let light = MaterialType::DiffuseLight;
    world.spheres.push(Sphere { position: Vector3::from([0.,10.,0.]), radius: 5., material: Material {material_type: light}});
